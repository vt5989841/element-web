...
export const useUnreadNotificationsFromAllRooms = () => {
    const [hasUnread, setHasUnread] = useState(false);
    const [rooms, setRooms] = useState<Room[]>([]);

    const checkUnreadFromAllRooms = useCallback((rooms: Room[]) => {
        const hasUnreadMessages = rooms.some((room) => {
            const { count, level } = determineUnreadState(room);
            return count > 0 && level > NotificationLevel.None;
        });
    ...
    useEffect(() => {
        // Check initial state
        checkUnreadFromAllRooms(rooms);

        // Listen for events that might change unread state
        const onEvent = () => checkUnreadFromAllRooms(rooms);
